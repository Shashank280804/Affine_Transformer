...
def upload_image():
    try:
        ...
        # Save the file to a temporary directory using tempfile
        temp_file_path = tempfile.mktemp(suffix='.png')
        file.save(temp_file_path)

        # Process the image using the transformation logic
        output_temp_path = tempfile.mktemp(suffix='.png')
        process_image(temp_file_path, output_temp_path, transformation, params)

        # Send the processed image as a response
        return send_from_directory(os.path.dirname(output_temp_path), os.path.basename(output_temp_path))
